...
export function JobMatchCard({
  ...
  return (
    <div className="rounded-2xl border border-slate-200/60 bg-white/70 backdrop-blur-xl shadow-[0_2px_12px_-4px_rgba(0,0,0,0.06)] hover:shadow-[0_8px_28px_-8px_rgba(37,99,235,0.12)] hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
      ...
      {/* ── Zone A + B: Score anchor + title/meta ── */}
      <div className="p-5 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {(company || location || seniority) && (
            <p className="mt-0.5 text-[12px] text-slate-400 flex flex-wrap items-center gap-x-1">
              {company && <span className="font-medium text-slate-500">{company}</span>}
              {company && location && <span>·</span>}
              {location && <span>{location}</span>}
              {seniority && (
                <>
                  <span>·</span>
                  <span className="capitalize">{seniority}</span>
                </>
              )}
            </p>
          )}
      ...
      {jobId && resumeId && (
        ...
        >
          ...
          <div>
            <GapAnalysisPanel jobId={jobId} resumeId={resumeId} mode="employee" />
          </div>
          <div>
            <MatchExplanationPanel jobId={jobId} resumeId={resumeId} mode="employee" />
          </div>
        </div>
      )}
